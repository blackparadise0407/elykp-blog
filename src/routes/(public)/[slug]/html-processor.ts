...
export const processHtml = (rawHTML: string) => {
  ...
  if (preloadDoc) {
    ...
    for (const codeBlock of codeBlocks) {
      ...
      button.className =
        'absolute btn btn-sm btn-square btn-primary top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity';
      button.innerHTML = `
      ...
      if (codeBlock.parentElement) {
        ...
        codeBlock.parentElement.outerHTML = `<div class="relative group">${codeBlock.parentElement.outerHTML} ${button.outerHTML}</div>`;
      }
    }
